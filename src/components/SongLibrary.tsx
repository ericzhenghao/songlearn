...
export default function SongLibrary({
  ...
  /* 云端歌曲数：用云端真实条数（同名本地歌会覆盖显示，但云端资产数量不变） */
  const cloudCount = cloudTotal ?? songs.filter((s) => s.source === "cloud").length;
      ...
      >
        ...
        <div className="border-b border-line-soft bg-ink-950/50 px-5 py-3">
          ...
          <div className="mt-1.5 flex items-center justify-between font-mono text-[10px] text-faint">
            ...
            <span title="索引只存引用；歌词与音频存在 Supabase 云端存储，公开直链，体积不计入索引">
              云端存储 · 单文件 ≤50MB
            </span>
          </div>
        </div>
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex shrink-0 flex-col gap-1.5">
                        ...
                        {onShare && s.source !== "cloud" && (
                          <button
                            onClick={() => onShare(s)}
                            className="rounded-md border border-sky/40 px-3 py-1 font-mono text-[10px] text-sky transition-colors hover:border-sky hover:bg-sky/10"
                            title="把这首歌同步进共享曲库，所有人（含微信打开的）都能看到"
                          >
                            ☁ 上云
                          </button>
                        )}
