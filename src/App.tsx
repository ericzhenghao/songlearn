...
import { deleteSong, findByTitleArtist, getSong, listSongs, newId, putSong, updateSong, type SongRecord } from "./lib/songdb";
...
const g2l = (g: GlobalSong): LibrarySong => ({
  ...
  source: "cloud",
  duration: g.duration,
  /* 歌词正文在 Supabase 直链上，打开歌曲时再拉；lineCount 让列表不拉全文也能显示进度 */
  lrc: g.lrc ?? "",
...
export default function App() {
  ...
  /* 无自定义库时默认进 main 共享库：任何设备打开都自动连上同一个库，曲库才是项目资产 */
  const [libId, setLibId] = useState<string | null>(() => loadLib() ?? DEFAULT_BIN);
   ...
   * 歌词正文 → Supabase 存储直链（.lrc 文件）；音频 → Supabase 存储直链（带进度）；
   * 共享表只存 {标题/歌手/两个直链/行数/热度}，约 250B/首。
   */
  ...
  /* 本地歌 → 同步进共享曲库（一键上云，让曲库跨设备积累） */
  async function shareLocalToCloud(s: LibrarySong) {
    if (!libId) {
      showToast("还没有共享曲库：先点「创建共享曲库」再上云");
      setLibraryOpen(true);
      return;
    }
    try {
      const rec = await getSong(s.id);
      if (!rec) {
        showToast("本地记录不存在，无法上云");
        return;
      }
      if (!rec.lrc || !rec.lrc.trim()) {
        showToast("这首歌没有歌词数据，无法共享");
        return;
      }
      const f = rec.fileBlob ? new File([rec.fileBlob], rec.fileName || `${rec.title}.mp3`, { type: rec.mime || "audio/mpeg" }) : null;
      await shareToGlobal(rec, f);
      await refreshGlobal();
    } catch (e) {
      showToast(e instanceof Error ? e.message : "上云失败");
    }
  }
  ...
  return (
    <div className="relative min-h-screen overflow-x-hidden bg-ink-950 font-body text-paper">
      ...
      <SongLibrary
        ...
        onShare={(s) => void shareLocalToCloud(s)}
        cloudTotal={globalSongs.length}
